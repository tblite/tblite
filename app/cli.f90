...
module tblite_cli
   ...
   use tblite_scf_mixer_input, only : mixer_input, anneal_input, scf_version, trial_version
...
subroutine get_run_arguments(config, list, start, error)
   ...
   logical :: getopts
   character(len=:), allocatable :: arg, sec, mixer_name, trial_name
   ...
   do while(iarg < narg)
      ...
      case("--mixer")
         ...
         if (ieq == 0) then
            mixer_sep = index(arg, ":")
            if (mixer_sep > 0) then
               if (mixer_sep == 1 .or. mixer_sep == len_trim(arg)) then
                  call fatal_error(error, "Invalid electronic mixer '"//arg//"' specified")
                  exit
               end if
               mixer_name = arg(:mixer_sep-1)
               trial_name = arg(mixer_sep+1:)
            else
               mixer_name = arg
            end if

            select case(mixer_name)
            case("broyden")
               config%mixer%scf = scf_version%broyden
            case default
               call fatal_error(error, "Unknown electronic mixer '"//mixer_name//"' specified")
               exit
            end select

            if (mixer_sep > 0) then
               select case(trial_name)
               case("oda")
                  config%mixer%trial = trial_version%oda
               case("mesa")
                  config%mixer%trial = trial_version%mesa
               case default
                  call fatal_error(error, "Unknown SCF mixer strategy '"//trial_name//"' specified")
                  exit
               end select
            else
               config%mixer%trial = trial_version%default
            end if
         else
            sec = arg(ieq+1:)
            ...
            case("trial-start")
               call get_argument_as_int(sec, config%mixer%trial_start, error)
            case default
               call fatal_error(error, "Unknown configuration key '"//arg(1:ieq-1)//"'")
               exit
            end select
      ...
      case("--mixer-trial-start")
         iarg = iarg + 1
         call list%get(iarg, arg)
         call get_argument_as_int(arg, config%mixer%trial_start, error)
         if (allocated(error)) exit
