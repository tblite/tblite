...
!subroutine get_hamiltonian(mol, overlap, dipole, hamiltonian)
subroutine get_hamiltonian(mol, trans, list, bas, h0, selfenergy, & 
   ...
   !$omp private(r2, vec, vec_diat_trafo, stmp, dtmpi, dtmpj, hij, rr, inl, img)
   do iat = 1, mol%nat
      izp = mol%id(iat)
      is = bas%ish_at(iat)
      inl = list%inl(iat)
   ...
   !$omp parallel do schedule(runtime) default(none) &
   !$omp shared(mol, bas, trans, cutoff2, overlap, overlap_diat, dpint, hamiltonian, h0, selfenergy) &
