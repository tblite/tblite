...
module tblite_xtb_singlepoint
   ...
   use tblite_scf, only : mixer_type, new_mixer, scf_info, next_outer_scf, &
      & get_mixer_dimension, potential_type, new_potential
   use tblite_scf_mixer_input, only : scf_version_to_string, trial_version_to_string, trial_version
...
subroutine xtb_singlepoint(ctx, mol, calc, wfn, accuracy, energy, gradient, sigma, &
   ...
   integer :: prlevel
   character(len=:), allocatable :: trial_info
   ...
   if (prlevel > 1) then
      trial_info = scf_version_to_string(calc%mixer_input%scf)
      if (calc%mixer_input%trial /= trial_version%default) then
         trial_info = trial_info // " with " // trial_version_to_string(calc%mixer_input%trial) // " trials"
      end if
      call ctx%message(trial_info)
   ...
   do while(.not.converged .and. iscf < calc%mixer_input%max_iter)
      elast = sum(eelec)
      ...
      call next_outer_scf(iscf, mol, calc%bas, wfn, solver, mixer, info, &
      ...
      if (prlevel > 0) then
         if (mixer%trial_evaluations /= iscf .and. prlevel > 1) then
            trial_info = " ("//format_string(mixer%trial_evaluations, "(i0)")//" evals)"
         else
            trial_info = ""
         end if
         call ctx%message(format_string(iscf, "(i7)") // &
            ...
            & escape(ctx%terminal%reset) // trial_info)
      end if
      if (allocated(error)) then
         call ctx%set_error(error)
         exit
