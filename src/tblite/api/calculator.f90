...
module tblite_api_calculator
   ...
   use tblite_scf_mixer_input, only : anneal_input, scf_version, trial_version
   ...
   public :: set_calculator_mixer_api, set_calculator_mixer_damping_api, &
      & set_calculator_mixer_memory_api, set_calculator_mixer_trial_start_api, &
      & set_calculator_max_iter_api, &
   ...
   enum, bind(c)
      enumerator :: &
         tblite_mixer_trial_default = 10_c_int, &
         tblite_mixer_trial_oda = 11_c_int, &
         tblite_mixer_trial_mesa = 12_c_int
   end enum
...
subroutine set_calculator_mixer_trial_start_api(vctx, vcalc, offset) &
      & bind(C, name=namespace//"set_calculator_mixer_trial_start")
   type(c_ptr), value :: vctx
   type(vp_context), pointer :: ctx
   type(c_ptr), value :: vcalc
   type(vp_calculator), pointer :: calc
   ...
   if (debug) print '("[Info]", 1x, a)', "set_calculator_mixer_trial_start"
   ...
   calc%ptr%mixer_input%trial_start = max(0, offset)
end subroutine set_calculator_mixer_trial_start_api


subroutine set_calculator_mixer_api(vctx, vcalc, mixer, trial) &
   ...
   select case(trial)
   case(tblite_mixer_trial_default)
      calc%ptr%mixer_input%trial = trial_version%default
   case(tblite_mixer_trial_oda)
      calc%ptr%mixer_input%trial = trial_version%oda
   case(tblite_mixer_trial_mesa)
      calc%ptr%mixer_input%trial = trial_version%mesa
   case default
      call fatal_error(error, "Unknown SCF mixer trial strategy selected")
      call ctx%ptr%set_error(error)
      return
   end select
