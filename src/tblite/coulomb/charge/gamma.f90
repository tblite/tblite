...
subroutine get_coulomb_matrix(self, mol, cache, amat)
   ...
   if (any(mol%periodic)) then
      ...
      if (self%ewald_matrix) then
         call get_amat_ewald_3d(mol, self%nshell, self%offset, cache%alpha, &
            & cache%vol, cache%rtrans, amat)
      end if
   else
...
subroutine get_amat_3d(mol, nshell, offset, hubbard, rcut, wsc, alpha, amat)
   ...
   real(wp) :: vec(3), ui, uj, wsw, dtmp, rtmp, aval
   ...
   !$omp shared(mol, nshell, offset, hubbard, wsc, dtrans, alpha, rcut) &
   !$omp private(iat, izp, jat, jzp, ii, jj, ish, jsh, ui, uj, wsw, vec, dtmp, aval)
   do iat = 1, mol%nat
      izp = mol%id(iat)
      ii = offset(iat)
      do jat = 1, iat-1
      ...
      do img = 1, wsc%nimg(iat, iat)
         ...
         do ish = 1, nshell(iat)
            do jsh = 1, ish-1
               ...
               uj = hubbard(jsh, izp)
               call get_amat_dir_3d(vec, ui, uj, alpha, dtrans, dtmp)
               aval = (dtmp - exp_gamma(0.0_wp, ui, uj)) * wsw
               !$omp atomic
            ...
            aval = (dtmp - exp_gamma(0.0_wp, ui, ui)) * wsw
            !$omp atomic
...
subroutine get_damat_3d(mol, nshell, offset, hubbard, rcut, wsc, alpha, qvec, ewald, &
      & dadr, dadL, atrace)
   !> Molecular structure data
   type(structure_type), intent(in) :: mol
   !> Number of shells for each atom
   ...
   !$omp shared(mol, wsc, alpha, vol, dtrans, rtrans, qvec, hubbard, nshell, offset, rcut, ewald) &
   ...
   if (ewald) then
      ...
      do iat = 1, mol%nat
         ...
         do jat = 1, iat-1
            ...
            jj = offset(jat)
            vec = mol%xyz(:, iat) - mol%xyz(:, jat)
            call get_damat_rec_3d(vec, vol, alpha, rtrans, dG, dS)
            do ish = 1, nshell(iat)
               do jsh = 1, nshell(jat)
                  itrace(:, ii+ish) = +dG*qvec(jj+jsh) + itrace(:, ii+ish)
                  itrace(:, jj+jsh) = -dG*qvec(ii+ish) + itrace(:, jj+jsh)
