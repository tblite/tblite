...
!> This writer produces a Molden input with the following the sections:
!>
!> ```
!> [Molden Format]
!> [Title]
...
!> ```
!> 
!> This format modifies the official Molden specification documented at
!>
!>   https://www.theochem.ru.nl/molden/molden_format.html
!> 
!> with an adapted atomic number excluding the core and the non-standard cell 
!> extension for periodic systems consistent Multiwfn and CP2K documented at
!> 
...
!> ```
!>
!> The non-standard `[Cell]` section specifies cartesian lattice vectors as:
!> 
!> ```
!> [Cell] AU
!>   ax ay az
...
!> ```
!>
!> Full three-dimensional periodicity is assumed when writing `[Cell]`.
...
contains
   ...
   subroutine molden_result(unit, mol, wfn, bas, error)
      ...
      ! Print symbols, atomic numbers excluding core electrons, and coordinates
      write(unit,'(A)') '[Atoms] AU'
      do iat = 1, mol%nat
         isp = mol%id(iat)
         ! Notation: symbol, atom, nuclear charge, coordinates
         write(unit,'(a,1x,i6,1x,i6,3(1x,ES24.16))') &
            & trim(mol%sym(isp)), iat, nint(wfn%n0at(iat)), &
         ...
         do imo = 1, bas%nao
            ...
            do iat = 1, mol%nat
               ...
               do ish = 1, bas%nsh_at(iat)
                  ...
                  call get_molden_cart_perm(li, perm)
   ...
   subroutine get_molden_cart_perm(l, perm)
