...
def test_gfn1_xtb_3d():
    ...
    forces = np.array(
        [
            [-0.06712660, -0.04953145, -0.05385737],
            [-0.04159200, -0.00664308, +0.01595108],
            [+0.01790311, -0.04489540, -0.01295220],
            [-0.00987778, +0.01964985, -0.03732586],
            [-1.92712865, -1.92624932, -1.93035419],
            [+1.97155953, +1.95281052, +1.96224849],
            [-1.89930581, -1.95539038, +1.94713977],
            [+1.95951570, +1.93966590, -1.93641261],
            [-1.93434774, +1.96147288, +1.94848549],
            [+1.94320828, -1.89625284, -1.95906558],
            [+1.93928139, -1.94413515, +1.95309129],
            [-1.95208944, +1.94949848, -1.89694830],
        ]
    )
    stress = np.array(
        [
            +0.0602884287181426e-0,
            +0.0602411116430432e-0,
            +0.0602545345924505e-0,
            +2.9882541705613027e-5,
            -1.6007594259106734e-5,
            +1.4593345729788693e-5,
        ]
    )
    ...
    assert approx(atoms.get_potential_energy(), abs=thr) == -1257.0943962462964
    assert approx(atoms.get_forces(), abs=thr) == forces
